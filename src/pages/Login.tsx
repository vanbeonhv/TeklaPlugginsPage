...
const Login = () => {
  ...
  const handleGithubLogin = async () => {
    try {
      ...
      await signIn('github@example.com', 'github123');
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      await signIn('google@example.com', 'google123');
  ...
  return (
    <div className='flex justify-center items-center flex-col bg-bright-blue-100 h-[calc(100vh-112px)]'>
      ...
      >
        <h3 className='text-3xl font-semibold text-center mb-10 text-slate-700 cursor-default'>Welcome back</h3>
        <button
          type='button'
        ...
        <div className=''>
          ...
          />
          <p className='min-h-[1rem] text-red-500 italic pl-2'>{errors.email?.message?.toString()}</p>
          <input
            type='password'
          ...
          />
          <p className='min-h-[1rem] text-red-500 italic pl-2'>{errors.password?.message?.toString()}</p>
          <div className='flex justify-between items-center text-sm text-slate-500 py-2'>
            ...
            <div className='underline cursor-pointer hover:text-slate-800'>Forgot password?</div>
          </div>
        </div>
